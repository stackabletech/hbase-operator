...
An alternative way to interact with HBase is to use the https://phoenix.apache.org/index.html[Phoenix] library that is pre-installed on the Stackable HBase image. Use the python utility `psql.py` to create, populate and query a table called `WEB_STAT`:
