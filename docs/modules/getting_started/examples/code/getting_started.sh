...
version() {
  ...
  curl -v -XGET -H "Accept: application/json" "http://simple-hbase-restserver-default:8080/version/cluster"
  # end::cluster-version[]
}
...
# tag::cluster-status[]
kubectl exec -n default simple-hbase-restserver-default-0 \
-- curl -v -XGET -H "Accept: application/json" "http://simple-hbase-restserver-default:8080/status/cluster" | json_pp
# end::cluster-status[]

# tag::create-table[]
kubectl exec -n default simple-hbase-restserver-default-0 \
-- curl -v -XPUT -H "Accept: text/xml" -H "Content-Type: text/xml" \
