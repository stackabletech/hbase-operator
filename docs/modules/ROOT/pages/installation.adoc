...
----
sudo apt-get update
sudo apt-get install stackable-hbase-operator
----
...
[source,bash]
----
sudo yum install stackable-hbase-operator
----
...
[source,bash]
----
docker run -e HBASE_OPERATOR_LOG=trace -e KUBECONFIG=/root/.kube/k3s.yaml --mount type=bind,source="$HOME/.kube/k3s.yaml",target="/root/.kube/k3s.yaml" -it docker.stackable.tech/stackable/hbase-operator:latest
----
